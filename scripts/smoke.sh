...
# BASE_URL debe apuntar a /api/v1
...
login () {
  ...
  curl -s -X POST "$BASE_URL/auth/login" \
    ...
    | jq -r '.token // .access_token // .plainTextToken // .data.token // empty'
}
...
ME_ADMIN="$(curl -s "$BASE_URL/auth/me" -H "Accept: application/json" -H "Authorization: Bearer $TOKEN_ADMIN")"
...
STATUS_BEFORE="$(curl -s -o /dev/null -w "%{http_code}" "$BASE_URL/modules/$SLUG" -H "Accept: application/json" -H "Authorization: Bearer $TOKEN_USER")"
...
STATUS_AFTER="$(curl -s -o /dev/null -w "%{http_code}" "$BASE_URL/modules/$SLUG" -H "Accept: application/json" -H "Authorization: Bearer $TOKEN_USER")"
...
DETAIL="$(curl -s "$BASE_URL/modules/$SLUG" -H "Accept: application/json" -H "Authorization: Bearer $TOKEN_USER")"
...
STATUS_ADMIN_DENY="$(curl -s -o /dev/null -w "%{http_code}" -X POST "$BASE_URL/admin/modules" \
